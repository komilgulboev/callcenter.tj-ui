...
const ROLES = [
  { value: 0, labelKey: 'role_operator' },
  { value: 1, labelKey: 'role_supervisor' },
  { value: 2, labelKey: 'role_hr' },
]
...
function CompanyForm({ form, setForm, tariffs }) {
  ...
  const handleTariffChange = (tariffId) => {
    ...
    if (tariffId) {
      const found = tariffs.find(t => t.id === parseInt(tariffId))
      if (found && found.maxOperators < 9999) set('maxUsers', found.maxOperators)
    }
  }
  ...
  return (
    <CRow className="g-3">
      ...
      <CCol md={6}>
        ...
        <CFormInput size="sm" value={form.businessProfile || ''} onChange={e => set('businessProfile', e.target.value)} />
      </CCol>
      <CCol md={6}>
        <CFormLabel style={lbl}>{t('field_representatives')}</CFormLabel>
        <CFormInput size="sm" value={form.representatives || ''} onChange={e => set('representatives', e.target.value)} />
      </CCol>
      <CCol md={6}>
        <CFormLabel style={lbl}>{t('field_rep_contact')}</CFormLabel>
        <CFormInput size="sm" value={form.representativesContact || ''} onChange={e => set('representativesContact', e.target.value)} />
      </CCol>
      <CCol md={6}>
      ...
      <CCol md={6}>
        ...
        <CFormInput size="sm" value={form.website || ''} onChange={e => set('website', e.target.value)} />
      </CCol>
      <CCol md={6}>
        <CFormLabel style={lbl}>{t('field_company_contact')}</CFormLabel>
        <CFormInput size="sm" value={form.companyContact || ''} onChange={e => set('companyContact', e.target.value)} />
      </CCol>
      <CCol md={6}>
        ...
        <CFormSelect size="sm" value={form.tariffId || ''} onChange={e => handleTariffChange(e.target.value)}>
          ...
          {tariffs.map(tr => (
            <option key={tr.id} value={tr.id}>
              {tr.name} — {tr.maxOperators >= 9999 ? '∞' : tr.maxOperators} {t('tariff_ops')} / ${tr.monthlyFee}/{t('tariff_month')}
            </option>
          ))}
        </CFormSelect>
      </CCol>
      ...
      <CCol md={12}>
        ...
        <CFormInput size="sm" value={form.location || ''} onChange={e => set('location', e.target.value)} />
      </CCol>
    </CRow>
  )
}
...
function CreateCompanyModal({ visible, onClose, onCreated }) {
  ...
  useEffect(() => {
    if (visible) api.getTariffs().then(t => setTariffs(Array.isArray(t) ? t : [])).catch(() => {})
  }, [visible])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onCreated(); onClose(); setForm(emptyForm)
    } catch { setError(t('error_create')) }
    ...
    <CModal visible={visible} onClose={onClose} size="lg">
      ...
      <CModalFooter>
        ...
        <CButton color="primary" size="sm" onClick={handleSave} disabled={saving}>
          {saving ? <CSpinner size="sm" className="me-1" /> : null}{t('btn_create')}
        </CButton>
      </CModalFooter>
    </CModal>
  )
...
function EditCompanyModal({ company, visible, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (visible && company) {
      setForm({
        name: company.name || '', businessProfile: company.businessProfile || '',
        representatives: company.representatives || '', taxId: company.taxId || '',
        representativesContact: company.representativesContact || '',
        website: company.website || '', companyContact: company.companyContact || '',
        location: company.location || '', maxUsers: company.maxUsers || 10,
  ...
  return (
    <CModal visible={visible} onClose={onClose} size="lg">
      ...
      <CModalFooter>
        <CButton color="secondary" variant="outline" size="sm" onClick={onClose}>{t('btn_cancel')}</CButton>
        <CButton color="primary" size="sm" onClick={handleSave} disabled={saving}>
          {saving ? <CSpinner size="sm" className="me-1" /> : null}{t('btn_save')}
        </CButton>
      </CModalFooter>
    </CModal>
  )
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderBottom: '1px solid #f0f0f0', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: 150 }}>
        ...
        <div style={{ fontSize: 11, color: '#6c757d' }}>@{user.username}{user.phone && <span style={{ marginLeft: 8 }}>📱 {user.phone}</span>}</div>
      </div>
      <div style={{ display: 'flex', gap: 6 }}>
        <CButton size="sm" color="success" disabled={loading} onClick={async () => { setLoading(true); try { await onActivate(user.id) } finally { setLoading(false) } }} style={{ fontSize: 12 }}>
          {loading ? <CSpinner size="sm" /> : `✅ ${t('activate')}`}
        </CButton>
        <CButton size="sm" color="danger" variant="outline" disabled={loading} onClick={async () => { setLoading(true); try { await onReject(user.id) } finally { setLoading(false) } }} style={{ fontSize: 12 }}>
          ✕ {t('reject')}
        </CButton>
      </div>
    </div>
...
// ─── USER ROW ───────────────────────────────────────────────
function UserRow({ user, onAssign, onUnassign, onRoleChange, companies, mode }) {
  const { t } = useTranslation('company')
  const fullName = [user.firstName, user.lastName].filter(Boolean).join(' ') || user.username
  ...
  const handleRoleChange = async (newRole) => {
    setRoleLoading(true)
    try { await onRoleChange(user.id, parseInt(newRole)) }
      ...
      {mode === 'unassigned' && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <CFormSelect size="sm" value={selectedCompany} onChange={e => setSelectedCompany(e.target.value)} style={{ fontSize: 12, minWidth: 160 }}>
            ...
            {companies.map(c => <option key={c.tenantId} value={c.tenantId}>{c.name}</option>)}
          </CFormSelect>
      ...
      {mode === 'assigned' && (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <div style={{ position: 'relative' }}>
            {roleLoading && <CSpinner size="sm" style={{ position: 'absolute', right: 24, top: 6, zIndex: 1 }} />}
            <CFormSelect
              size="sm"
              value={user.role ?? 0}
              onChange={e => handleRoleChange(e.target.value)}
              disabled={roleLoading}
              style={{ fontSize: 11, minWidth: 120, paddingRight: roleLoading ? 32 : undefined }}
            >
              {ROLES.map(r => (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          {company.representatives && <div style={{ fontSize: 11, color: '#6c757d' }}>👤 {company.representatives}</div>}
          {company.tariffName && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
          <div style={{ display: 'flex', gap: 4 }}>
            <CButton size="sm" color="light" onClick={e => { e.stopPropagation(); onEdit(company) }} style={{ fontSize: 11, padding: '2px 8px' }}>✏️</CButton>
            <CButton size="sm" color={company.status ? 'warning' : 'success'} variant="outline" disabled={toggling}
              onClick={e => { e.stopPropagation(); onToggle(company) }} style={{ fontSize: 11, padding: '2px 8px', whiteSpace: 'nowrap' }}>
          ...
          <div style={{ fontSize: 12, color: '#495057', fontWeight: 600 }}>
            ...
            {company.tariffMaxOperators && company.tariffMaxOperators < 9999
              ? <span style={{ color: '#adb5bd', fontWeight: 400 }}>/{company.tariffMaxOperators}</span> : null}
          </div>
        </div>
      </div>
    </div>
...
export default function Companies() {
  ...
  const handleRoleChange = async (userId, role) => {
    ...
    setCompanyUsers(prev => prev.map(u => u.id === userId ? { ...u, role } : u))
  }

  const handleActivate = async (userId) => { await api.activateUser(userId); await load() }
  const handleReject   = async (userId) => { await api.rejectUser(userId);   await load() }
            ...
            <CCardBody style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '14px 16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <CButton size="sm" color="primary" onClick={() => setShowCreate(true)} style={{ fontSize: 12 }}>+ {t('new_company')}</CButton>
                </div>
              </div>

              <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexShrink: 0 }}>
                <CButton size="sm" color={tab === 'companies' ? 'primary' : 'light'} onClick={() => { setTab('companies'); setSearch('') }} style={{ fontSize: 12 }}>
                  🏢 {t('tab_companies')} ({companies.length})
                </CButton>
                <CButton size="sm" color={tab === 'unassigned' ? 'warning' : 'light'} onClick={() => { setTab('unassigned'); setSearch(''); setSelected(null) }} style={{ fontSize: 12 }}>
                  👤 {t('tab_unassigned')} ({unassigned.length})
                </CButton>
                <CButton size="sm" color={tab === 'pending' ? 'danger' : 'light'} onClick={() => { setTab('pending'); setSearch(''); setSelected(null) }} style={{ fontSize: 12 }}>
              ...
              {assignError && (
                <div style={{ background: '#fff3cd', color: '#664d03', borderRadius: 6, padding: '8px 12px', fontSize: 12, marginBottom: 8, flexShrink: 0, display: 'flex', justifyContent: 'space-between' }}>
                  {assignError}<span style={{ cursor: 'pointer' }} onClick={() => setAssignError(null)}>✕</span>
                </div>
              )}
              ...
              <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
                {tab === 'companies' && filteredCompanies.map(c => (
                  <CompanyCard key={c.id} company={c} selected={selected}
                    onClick={() => selected?.id === c.id ? setSelected(null) : loadCompanyUsers(c)}
                    onEdit={setEditCompany} onToggle={handleToggle} toggling={togglingId === c.id} />
                ))}
                  ...
                  filteredUnassigned.length === 0
                    ...
                    : filteredUnassigned.map(u => (
                        <UserRow key={u.id} user={u} companies={companies} mode="unassigned"
                          onAssign={handleAssign} onUnassign={handleUnassign} onRoleChange={handleRoleChange} />
                      ))
                )}
              ...
              <div style={{ paddingTop: 8, textAlign: 'center', fontSize: 11, color: '#adb5bd', flexShrink: 0 }}>
                {tab === 'companies' ? `${filteredCompanies.length} ${t('tab_companies').toLowerCase()}`
                  : tab === 'unassigned' ? `${filteredUnassigned.length} ${t('tab_unassigned').toLowerCase()}`
